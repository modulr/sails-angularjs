    ...
    .controller('ProfileCtrl', ['$rootScope', '$scope', '$state', '$location', '$translate', 'restFulService', 'config', 'errorService', function($rootScope, $scope, $state, $location, $translate, restFulService, config, errorService){
        ...
        */
        ...
        $scope.savePersonal = function(event)
        {
            ...
            $scope.formPersonal.gender = $scope.formPersonal.select.gender.gender;
            $scope.formPersonal.relationship = $scope.formPersonal.select.relationship.relationship;
                ...
                .then(function(response){
                    ...
                    $scope.user.contact.forEach(function(val, key) {
                        if (val.id == id) {
                            ...
                            $scope.user.contact[key].type = $scope.formContact.select.type.type;
                            return false;
                        }
                    });
        ...
        $scope.saveAccount = function(event)
        {
            if($('#formAccount').smkValidate()){
                ...
                restFulService.put('user/updateAccount/' + $scope.user.id, $scope.formAccount)
                .then(function(response){

                    if ($scope.formAccount.email !== response.email) {
                        $translate('PROFILE.MESSAGES.EMAIL-CHANGE.INFO').then(function (translate) {
                            $.smkAlert({
                                text: translate,
                                type: 'info',
                                position: 'bottom-left',
